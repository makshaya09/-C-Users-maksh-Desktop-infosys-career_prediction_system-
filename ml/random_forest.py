"""
Random Forest Classifier Pipeline with Cross-Validated Hyperparameter Tuning.
...
from ml.config import RF_MODEL_DIR, RF_PARAM_GRID, DATA_DIR, RESULTS_DIR


def load_dataset(dataset_path: str = None) -> pd.DataFrame:
    """Loads and cleans dataset for ML training."""
...
def train_random_forest(
    ...
    print("[Random Forest] 1/5 Loading dataset...")
    ...
    print("[Random Forest] 2/5 Splitting dataset (80% train, 20% test)...")
    ...
    print("[Random Forest] 3/5 Vectorizing text with TF-IDF...")
    ...
    print(f"[Random Forest] 4/5 Running GridSearchCV ({cv_folds}-fold Stratified CV)...")
    ...
    print("[Random Forest] 5/5 Evaluating best model on test set...")
    ...
    meta_path = os.path.join(model_dir, "rf_metrics.json")
    ...
    print(f"[Random Forest] Test Accuracy : {accuracy * 100:.2f}% | F1-Score: {f1 * 100:.2f}%")
    print(f"[Random Forest] Model and artifacts saved to: {model_dir}")
